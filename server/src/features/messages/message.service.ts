...
export class MessageService {
  async send(input: SendMessageInput): Promise<IMessage> {
    ...
    const conv = await Conversation.findOne({ _id: convId, participants: sndId, isDeleted: false });
  ...
  async getMessages(
    ...
    const conv = await Conversation.findOne({ _id: convId, participants: uid, isDeleted: false });
  ...
  async markDelivered(conversationId: string, userId: string): Promise<void> {
    ...
    await Message.updateMany(
      { conversation: new mongoose.Types.ObjectId(conversationId), deliveredTo: { $ne: uid } },
  ...
  async deleteForEveryone(messageId: string, userId: string): Promise<void> {
    const msg = await Message.findOne({ _id: messageId, sender: new mongoose.Types.ObjectId(userId) });
  ...
  async searchMessages(conversationId: string, userId: string, query: string): Promise<IMessage[]> {
    const convId = new mongoose.Types.ObjectId(conversationId);
    const uid = new mongoose.Types.ObjectId(userId);
