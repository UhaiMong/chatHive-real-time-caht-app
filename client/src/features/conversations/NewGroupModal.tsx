...
import { setActiveConversation, setNewGroupModalOpen } from '../../shared/store/uiSlice';
import { addNotification } from '../notifications/notificationsSlice';
import type { User } from '../../shared/types';

interface NewGroupModalProps { open: boolean; }
...
export const NewGroupModal = ({ open }: NewGroupModalProps) => {
  ...
  const handleCreate = async () => {
    if (!groupName.trim() || selected.length < 2) {
      dispatch(addNotification({ type: 'warning', message: 'Group needs a name and at least 2 members' }));
      return;
    }
    setSubmitting(true);
    try {
      ...
      <div className="p-4 flex flex-col gap-4">
        ...
        {selected.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {selected.map(u => (
              <span key={u._id} className="flex items-center gap-1.5 bg-primary-600/20 text-primary-300 text-xs rounded-full pl-2.5 pr-1.5 py-1">
                {u.username}
                <button onClick={() => toggle(u)} className="hover:text-white transition-colors">
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors text-left ${
                  isSelected ? 'bg-primary-600/10' : 'hover:bg-surface-elevated'
                }`}
              >
