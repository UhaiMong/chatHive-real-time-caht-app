...
export const Sidebar = memo(() => {
  ...
  const { items: convs, status, typingUsers } = useAppSelector(s => s.conversations);
  const { activeConversationId, sidebarOpen, searchQuery, newChatModalOpen, newGroupModalOpen } =
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <motion.aside
            ...
            className={cn(
              'flex flex-col h-full w-full md:w-[340px] flex-shrink-0',
            ...
            <div className="flex-1 overflow-y-auto px-2 py-1 space-y-0.5">
              ...
              {status === 'succeeded' && filtered.length === 0 && (
                <EmptyState
                  ...
                  description={searchQuery ? 'Try a different search' : 'Start a new chat or group'}
                />
              )}
              <AnimatePresence>
                {filtered.map(conv => {
                  ...
                  const typingNames = typingIds
                    .map(id => conv.participants.find(p => p._id === id)?.username)
