...
export const ConversationItem = memo(({
  ...
  const lastMsgPreview = useMemo(() => {
    ...
    if (msg.type !== 'text') {
      const icons: Record<string, string> = { image: '📷 Photo', file: '📎 File', audio: '🎵 Audio', video: '🎬 Video' };
      return icons[msg.type] ?? '📎 Attachment';
    }
    const isMine = msg.sender._id === currentUserId;
    const prefix = isMine ? 'You: ' : (conversation.type === 'group' ? `${msg.sender.username}: ` : '');
    return `${prefix}${msg.content}`;
  }, [conversation.lastMessage, typingUsernames, currentUserId, conversation.type]);
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center justify-between gap-1 mt-0.5">
          {typingUsernames.length > 0 ? (
            ...
            <span className="text-xs text-gray-500 truncate">{lastMsgPreview}</span>
          )}

          {(conversation.unreadCount ?? 0) > 0 && (
            <span className="flex-shrink-0 min-w-[18px] h-[18px] px-1 bg-primary-600 rounded-full text-[10px] font-bold text-white flex items-center justify-center">
