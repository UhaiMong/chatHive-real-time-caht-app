import {
  useState, useRef, useCallback, KeyboardEvent, ChangeEvent, memo,
} from 'react';
import {
  PaperAirplaneIcon, PaperClipIcon, FaceSmileIcon, XMarkIcon,
...
export const MessageInput = memo(({
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected.size > MAX_FILE_SIZE) {
      dispatch(addNotification({ type: 'error', message: 'File exceeds 10MB limit' }));
      return;
    }
    setFile(selected);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      } catch { /* socket not critical here, HTTP already saved */ }

      dispatch(addMessage(newMsg));
    } catch {
      dispatch(addNotification({ type: 'error', message: 'Failed to send message' }));
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <div className="flex-shrink-0 bg-surface-secondary border-t border-white/5">
      {/* Edit mode banner */}
      {editingMessage && (
        <div className="flex items-center gap-3 px-4 py-2 border-b border-white/5">
          ...
          <button onClick={onCancelEdit} className="p-1.5 rounded-lg hover:bg-surface-elevated text-gray-500 hover:text-gray-300 transition-colors">
        ...
        {file && (
          ...
          >
            <div className="flex items-center gap-3 bg-surface-elevated rounded-xl p-3 max-w-xs">
              ...
              <button onClick={clearFile} className="p-1 rounded-lg hover:bg-surface text-gray-500 hover:text-gray-300 transition-colors flex-shrink-0">
                <XMarkIcon className="w-4 h-4" />
              </button>
            </div>
          </motion.div>
      ...
      <div className="flex items-end gap-2 px-3 py-3">
        ...
        <button
          ...
          className="p-2.5 rounded-full hover:bg-surface-elevated text-gray-400 hover:text-gray-200 transition-colors flex-shrink-0"
          title="Attach file"
        >
        ...
        <div className="relative flex-shrink-0">
          <button
            ...
            className="p-2.5 rounded-full hover:bg-surface-elevated text-gray-400 hover:text-gray-200 transition-colors"
            title="Emoji"
          >
        ...
        <textarea
          ...
          className={cn(
            'flex-1 bg-surface-input rounded-2xl px-4 py-2.5 text-sm text-gray-100',
