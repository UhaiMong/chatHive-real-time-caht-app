...
const DateDivider = ({ date }: { date: string }) => {
  ...
  return (
    <div className="flex items-center gap-3 py-2 px-4">
      ...
      <span className="text-[11px] text-gray-600 flex-shrink-0 bg-surface px-2 py-0.5 rounded-full">
        {label}
      </span>
      <div className="flex-1 h-px bg-white/5" />
    </div>
...
export const MessageList = memo(({
  ...
  const pageData = useAppSelector(s => s.messages.byConversation[conversationId]);
  ...
  const { ref: topSentinel, inView: topInView } = useInView({ threshold: 0.1 });
  ...
  const groupedMessages = messages.reduce<{ date: string; items: Message[] }[]>((acc, msg, i) => {
  ...
  return (
    <div className="relative flex-1 min-h-0">
      <div
        ...
        style={{ backgroundImage: 'radial-gradient(ellipse at 50% 0%, rgba(34,197,94,0.02) 0%, transparent 60%)' }}
      >
        {/* Load more sentinel */}
