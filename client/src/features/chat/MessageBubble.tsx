...
import {
  PencilIcon, TrashIcon, ArrowUturnLeftIcon,
  DocumentIcon, CheckIcon, CheckCircleIcon,
} from '@heroicons/react/24/outline';
import { cn, formatMessageTime, formatFileSize } from '../../shared/utils/helpers';
...
export const MessageBubble = memo(({
  ...
  return (
    ...
    >
      ...
      <div className={cn('flex flex-col max-w-[70%] min-w-0', isMine ? 'items-end' : 'items-start')}>
            ...
            <div className="mb-2">
              {message.media.type === 'image' && (
                <img
                  ...
                  className="rounded-xl max-w-[260px] max-h-[260px] object-cover cursor-pointer hover:opacity-90 transition-opacity"
                  loading="lazy"
              ...
              {message.media.type === 'audio' && (
                <audio src={message.media.url} controls className="max-w-[240px]" />
              )}
              {message.media.type === 'file' && (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <p className="text-xs font-medium text-gray-200 truncate max-w-[160px]">
                      {message.media.name}
                    </p>
                    <p className="text-[10px] text-gray-400">{formatFileSize(message.media.size)}</p>
                  </div>
                </a>
              )}
            </div>
        ...
        <div className={cn(
          ...
          {isMine && (
            <ActionBtn
              ...
              title="Delete"
              onClick={() => emitDeleteMessage(message._id, conversationId, 'all')}
              danger
            />
          )}
        </div>
