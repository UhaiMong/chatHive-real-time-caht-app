...
export const ChatWindow = memo(({ conversation, currentUserId }: ChatWindowProps) => {
  ...
  const handleEditSubmit = useCallback(async (messageId: string, content: string) => {
    try {
      const res = await messagesApi.editMessage(conversation._id, messageId, content);
      dispatch(updateMessage(res.data.data!));
    } catch {
      dispatch(addNotification({ type: 'error', message: 'Failed to edit message' }));
    }
    setEditingMessage(null);
