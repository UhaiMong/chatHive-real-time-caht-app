...
import {
  Bars3Icon, MagnifyingGlassIcon, PhoneIcon, EllipsisVerticalIcon,
...
export const ChatHeader = memo(({ conversation, currentUserId }: ChatHeaderProps) => {
  const dispatch = useAppDispatch();
  const typingUsers = useAppSelector(s => s.conversations.typingUsers[conversation._id] ?? []);
  ...
  const subtitle = useMemo(() => {
    if (typingUsers.length > 0) {
      const names = typingUsers
        .map(id => conversation.participants.find(p => p._id === id)?.username)
        .filter(Boolean);
    ...
    if (conversation.type === 'direct' && peer) {
      ...
      return peer.lastSeen ? `last seen ${formatLastSeen(peer.lastSeen)}` : 'offline';
    }
  ...
  return (
    <header className="flex items-center gap-3 px-4 py-3 bg-surface-secondary border-b border-white/5 flex-shrink-0">
      <button
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className={`text-xs truncate transition-colors ${
            typingUsers.length > 0 ? 'text-primary-400 italic' : 'text-gray-500'
          }`}>
            {subtitle}
          </p>
        </div>
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        <button className="p-2 rounded-lg hover:bg-surface-elevated text-gray-400 hover:text-gray-200 transition-colors opacity-40 cursor-not-allowed" title="Voice call (coming soon)">
          <PhoneIcon className="w-5 h-5" />
        </button>
        <button className="p-2 rounded-lg hover:bg-surface-elevated text-gray-400 hover:text-gray-200 transition-colors" title="Search">
